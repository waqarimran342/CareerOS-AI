...
- Client: Qwen client wrapper for LLM calls with robust JSON parsing and retry behavior.
...
subgraph "LLM Client"
J["QwenClient<br/>chat_json, extract_json_object"]
end
subgraph "Config"
...
## Core Components
- Agents: Each agent encapsulates a focused task, builds a prompt, calls Qwen via chat_json, and returns a structured dict.
- Orchestrator: run_full_analysis constructs agents, executes stages, and aggregates outputs.
- QwenClient: Wraps OpenAI-compatible API calls, enforces strict JSON output rules, and retries once on malformed responses.
...
**Section sources**
- [agents.py:29-289](file://src/agents.py#L29-L289)
- [agents.py:295-334](file://src/agents.py#L295-L334)
...
- Stage 1 & 2: Independent analyses of resume and GitHub evidence. These can be conceptually parallelized since they do not depend on each other.
...
- Error propagation:
  ...
  - QwenError is caught and converted to HTTP 502.
...
- Pattern:
  - Create a new agent class with a name attribute and a run method that builds prompts and calls qwen.chat_json.
...
- LLM-level errors:
  - QwenError: Raised for API failures or repeated invalid JSON; mapped to HTTP 502.
- Validation errors:
  - Missing or invalid inputs raise HTTP 400 before invoking the pipeline.
- Testing:
  - Offline tests use FakeQwen to simulate agent calls and assert pipeline order and output structure without network access.
...
## Dependency Analysis
- Coupling:
  - Orchestrator depends on all agents and QwenClient.
  - Agents depend only on QwenClient and their specific inputs.
...
- Retry behavior:
  - QwenClient retries once on malformed JSON to improve robustness without excessive retries.
- Timeouts:
  - Configurable timeouts for Qwen and GitHub calls prevent hanging requests.
- Concurrency:
...
- Missing or invalid API key:
  - Symptom: QwenError during initialization or chat_json.
  - Resolution: Ensure DASHSCOPE_API_KEY is set in .env and accessible to the app.
- GitHub rate limit:
...
- Pipeline order verification:
  - Use offline tests with FakeQwen to assert agent call order and output structure.
...
- Offline tests validate:
  ...
  - QwenClient initialization behavior.
  - GitHub summary construction with fixtures.
  - Resume service error paths with generated PDFs.
  - Full pipeline execution order and output structure using FakeQwen.
