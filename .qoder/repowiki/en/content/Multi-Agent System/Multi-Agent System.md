...
## Update Summary
**Changes Made**
- Updated to reflect complete five-agent system implementation with ResumeAnalysisAgent, GitHubEvidenceAgent, JobMatchingAgent, SkillGapAgent, and MasterCareerAgent
- Corrected LLM client from Qwen to Google Gemini (google-generativeai SDK)
- Updated model configurations and environment variables for Gemini
- Enhanced pipeline orchestration details with specific agent coordination patterns
- Added comprehensive evidence-based assessment methodology documentation
- Updated API references and configuration settings
...
The pipeline uses structured JSON communication between agents via a Google Gemini client wrapper, with configuration-driven model parameters and robust error handling.
...
- LLM client abstraction over Google Gemini
...
A --> C["Gemini Client<br/>src/qwen_client.py"]
...
- **Five agents**: each encapsulates a focused prompt, calls Gemini via structured JSON, and returns typed dictionaries consumed by downstream agents.
- **Gemini client**: Google Generative AI wrapper with retry on invalid JSON output and strict parsing.
...
- [agents.py:27-337](file://src/agents.py#L27-L337)
- [qwen_client.py:74-161](file://src/qwen_client.py#L74-L161)
...
API->>AG : run_full_analysis(gc, resume_text, github_profile, target_role, job_description)
AG->>GC : chat_json (Resume Analysis Agent)
GC-->>AG : resume_analysis
AG->>GC : chat_json (GitHub Evidence Agent)
GC-->>AG : github_analysis
AG->>GC : chat_json (Job Matching Agent)
GC-->>AG : job_match
AG->>GC : chat_json (Skill Gap Agent)
GC-->>AG : skill_gaps
AG->>GC : chat_json (Master Career Agent)
GC-->>AG : career_report
...
- **Communication protocol**:
  - Each agent constructs a system prompt and user prompt, then calls GeminiClient.chat_json
...
- **Cross-references**: Claims vs verified skills to avoid inflated matches
...
### Gemini Client and Model Parameters
- **Abstraction**: Thin wrapper around Google Generative AI SDK pointing to Gemini models
- **Model configuration**:
  - Model: gemini-3.6-flash by default (configurable via GEMINI_MODEL)
  - Temperature: configurable (default 0.7) for controlled randomness
  - Max tokens: configurable (default 2048), overridden per agent when needed
  - Master Career Agent overrides max_tokens to 8192 for longer synthesis
- **Error handling**:
  - Raises GeminiError on network/auth/rate-limit issues
...
- Health endpoint reports app status, configured model, and whether Gemini/GitHub credentials are set
- Analysis endpoint:
...
- **LLM call costs and latency**:
  - Configurable temperature and max tokens help control cost and improve determinism
  - Master Career Agent overrides max_tokens for longer synthesis (8192 tokens)
- **External API constraints**:
  - GitHub rate limits handled gracefully; recommend setting GITHUB_TOKEN for higher limits
  - Gemini timeouts configurable to balance responsiveness and reliability
...
- **Missing Gemini API key**:
  - Symptom: GeminiError raised during initialization or analysis
  - Resolution: Set GOOGLE_API_KEY in .env and ensure base URL is correct
...
- Call gc.chat_json with the prompts; leverage shared JSON rules for consistent output
- Integrate the agent into run_full_analysis by inserting it at the appropriate stage and wiring its inputs/outputs
- Update tests to include the new agent's expected response in the FakeGemini sequence
...
- **GEMINI_MODEL**: Default gemini-3.6-flash; can be tuned to other Gemini models
- **GEMINI_TEMPERATURE**: Controls randomness; default 0.7
- **GEMINI_MAX_TOKENS**: Default 2048; can be overridden per agent
...
### Dependencies
- fastapi, uvicorn[standard], google-generativeai, python-dotenv, requests, pypdf, python-multipart, pydantic
